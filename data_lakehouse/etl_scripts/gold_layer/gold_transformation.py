...
# MAGIC
# MAGIC The script transforms Silver layer data into Gold layer dimension and fact tables in a Delta Lakehouse.
# MAGIC
# MAGIC - dim_customers → created from streaming customers data, cleaned and enriched with standardized city/country names.
# MAGIC
# MAGIC - dim_products → built from static products data with filtering and normalization.
# MAGIC
# MAGIC - fact_sales → generated by joining orders with customer and product dimensions, calculating total_order_value, and filtering invalid records.
# MAGIC
# MAGIC All tables are stored in the pyspark_dwh.gold schema with Delta format, checkpoints for streaming consistency, and ZORDER optimization for faster queries.
